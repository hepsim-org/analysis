 ...
 * Copyright (C) 2002 Michiel Jan Laurens de Hoon S Chekanov (ANL).
 *
...
// S.Chekanov - to constrain configuration 
...
void CALL sort(int n, double data[], int index[])
...
{ int i, j;
  ...
  if (dist=='s' || dist=='k')
  { double* rank = (double*)malloc(ncolumns*sizeof(double));
    double* row = (double*)malloc(ncolumns*sizeof(double));
    for (i = 0; i < nrows; i++)
    { int n = 0;
...
// silimilar to Euclidian, only in eta and phi
// phi is closed
// assume that variable were normilized

static
double etaphi (int n, double** data1, double** data2, int** mask1, int** mask2,
  const double weight[], int index1, int index2, int transpose)
{ double result = 0.;
  double tweight = 0;
  int i;
...
//        double term = data1[index1][i] - data2[index2][i];
         ...
         // eta
         if (i == 0) term=fabs(data1[index1][i] - data2[index2][i]); 
         // phi
         if (i == 1) {
                     term=fabs(data1[index1][i] - data2[index2][i]); 
                     // if (term>pi) term=pi2-term;
                      // assume normilisation 
                     if (term>0.5) term=1.0-term;
         }
...
//         double term = data1[i][index1] - data2[i][index2];
         // first eta
         if (i == 0) term=fabs(data1[i][index1] - data2[i][index2]);
         // phi
         if (i == 1) {
                     term=fabs(data1[i][index1] - data2[i][index2]);
                      // if (term>pi) term=pi2-term;
                      // asume normalisation 
                       if (term>0.5) term=1.0-term;
         }
...
{ switch(dist)
  { case ('e'): *metric = &euclid; break;
    case ('r'): *metric = &etaphi; break; // added by S.Chekanov  
...
dist=='r': Euclidean distance in Eta and Phi (phi closed) 
...
{ const int nelements = (transpose==0) ? nrows : ncolumns;
  ...
  *ifound = 1;
  ...
  if (transpose)
  { double** newdata = (double**)malloc((nelements+nclusters)*sizeof(double*));
    int** newmask = (int**)malloc((nelements+nclusters)*sizeof(int*));
    for (i = 0; i < nelements; i++)
    { newdata[i] = (double*)malloc(ndata*sizeof(double));
      newmask[i] = (int*)malloc(ndata*sizeof(int));
      for (j = 0; j < ndata; j++)
    ...
    for (i = 0; i < nclusters; i++)
    { newdata[nelements+i] = (double*)malloc(ndata*sizeof(double));
      newmask[nelements+i] = (int*)malloc(ndata*sizeof(int));
    }
    data = newdata;
    mask = newmask;
  }
...
// impose some constrain on this configuration 
// input: data matrix and cluster ID
// return: weigth for this configuration 
// if no constrain sets, then wgt=1.0
// most probably event correspond small valuee of wgt;
// most unprobably to the large valy (>>1)


  double wgt=1.0;   
//  wgt=checkconfig(nclusters,data,nrows,ncolumns,tclusterid);
//  if (tssin == 0.0) tssin=9999999.0; 
//  tssin = wgt*tssin; 
//  printf("k-means clustering:  attempt %d passed with distance sum %12.5lf \n", ipass, tssin);
...
{ double (*metric)
  ...
  { double** newdata = (double**)malloc(nelements*sizeof(double*));
    int** newmask = (int**)malloc(nelements*sizeof(int*));
    for (i = 0; i < nelements; i++)
    { newdata[i] = (double*)malloc(ndata*sizeof(double));
      newmask[i] = (int*)malloc(ndata*sizeof(int));
      for (j = 0; j < ndata; j++)
...
{ int j;
  ...
  int* clusterid = (int*)malloc(nelements*sizeof(int));
...
{ int j;
  ...
  int* number = (int*)malloc(nelements*sizeof(int));
  /* Setup a list specifying to which cluster a gene belongs */
  int* clusterid = (int*)malloc(nelements*sizeof(int));
